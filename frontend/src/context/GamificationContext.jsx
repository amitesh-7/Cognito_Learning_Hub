...
export const GamificationProvider = ({ children }) => {
    ...
    const handleAchievementUnlocked = (data) => {
      ...
      setUserAchievements((prev) => {
        const exists = prev.some((a) => 
          (a.achievement?._id || a._id) === (achievement._id || achievement.id)
        );
        if (exists) return prev;
        return [...prev, { achievement, unlockedAt: new Date(), isCompleted: true }];
      });
  ...
  const checkAchievements = useCallback(async (eventData) => {
  ...
  const getLevelProgress = useCallback(() => {
    ...
    // XP required formula: level * 100 (simple progression)
