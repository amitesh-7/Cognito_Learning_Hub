...
export const AvatarProvider = ({ children }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          setEmotionalState(data.avatar.emotionalState || { mood: "neutral", intensity: 50 });
        }
  ...
  const updateAvatar = useCallback(async (updates) => {
    try {
      const token = localStorage.getItem("quizwise-token");
      const response = await fetch(`${API_URL}/api/avatar`, {
        method: "PUT",
  ...
  const triggerReaction = useCallback(async (event, context = {}) => {
    try {
      const token = localStorage.getItem("quizwise-token");
  ...
  const addExperience = useCallback(async (amount, source, quizStats = null) => {
    try {
      const token = localStorage.getItem("quizwise-token");
      const response = await fetch(`${API_URL}/api/evolution/add-experience`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const getReaction = useCallback(async (event) => {
    try {
      const token = localStorage.getItem("quizwise-token");
      const response = await fetch(`${API_URL}/api/avatar/reaction/${event}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const analyzeLearningStyle = useCallback(async (quizData) => {
    try {
      const token = localStorage.getItem("quizwise-token");
  ...
  const generateVoiceExplanation = useCallback(async (text, questionContext = {}) => {
    try {
      const token = localStorage.getItem("quizwise-token");
      const response = await fetch(`${API_URL}/api/voice/generate-explanation`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
