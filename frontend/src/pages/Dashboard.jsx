...
import { Award, CheckCircle, Target, ClipboardList, TrendingUp, Calendar, Flame, AlertTriangle, BookOpen, Clock, User, Eye, EyeOff } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/results/my-results`, {
  ...
  const averageScore = quizzesCompleted > 0 
    ? results.reduce((acc, r) => acc + (r.score / r.totalQuestions) * 100, 0) / quizzesCompleted
    : 0;
  ...
  const excellent = results.filter(r => (r.score / r.totalQuestions) >= 0.9).length;
  const good = results.filter(r => (r.score / r.totalQuestions) >= 0.7 && (r.score / r.totalQuestions) < 0.9).length;
  const needsWork = results.filter(r => (r.score / r.totalQuestions) < 0.7).length;

  // --- Chart Data ---
  const chartData = results.slice(0, 10).reverse().map((r, index) => ({
    name: `Quiz ${index + 1}`,
  ...
  // --- Achievements Logic ---
  const achievements = [];
  if (quizzesCompleted > 0) achievements.push({ 
    icon: Award, 
    title: "First Quiz!", 
  ...
  if (quizzesCompleted >= 5) achievements.push({ 
    icon: Target, 
    title: "Quiz Novice", 
  ...
  if (quizzesCompleted >= 10) achievements.push({ 
    icon: CheckCircle, 
    title: "Quiz Expert", 
  ...
  if (results.some(r => r.score === r.totalQuestions)) achievements.push({ 
    icon: Flame, 
    title: "Perfectionist", 
  ...
  if (streakCount >= 3) achievements.push({ 
    icon: Calendar, 
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      >
        ...
        <p className="text-gray-600 dark:text-gray-400">Preparing your learning insights...</p>
      </motion.div>
    </div>
  );
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-red-50 via-white to-pink-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      >
        <Card className="p-8 bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg border-0 shadow-xl">
          ...
          <Button onClick={() => window.location.reload()} className="bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700">
            Try Again
          </Button>
        </Card>
      </motion.div>
        ...
        >
          ...
          <div className="flex gap-3">
            <Button 
              ...
              className={viewMode === 'overview' ? 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700' : ''}
            >
            ...
            <Button 
              ...
              className={viewMode === 'detailed' ? 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700' : ''}
            >
            ...
            >
              ...
              <motion.div className="space-y-6" variants={itemVariants}>
                ...
                <Card className="text-center p-8 bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg border-0 shadow-xl">
                  ...
                  <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">{user?.name}</h2>
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div className="grid grid-cols-2 gap-6 text-center">
                      <div className="p-4 rounded-xl bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900 dark:to-purple-900">
                        <p className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">{quizzesCompleted}</p>
                      ...
                      <div className="p-4 rounded-xl bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900 dark:to-emerald-900">
                        <p className="text-3xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">{averageScore.toFixed(0)}%</p>
                ...
                {/* Achievements */}
                <Card className="p-6 bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg border-0 shadow-xl">
                  <h3 className="text-lg font-semibold bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-300 bg-clip-text text-transparent mb-6">Recent Achievements</h3>
                        ...
                        return (
                          ...
                          >
                            <div className={`p-3 rounded-xl ${
                              ach.color === 'yellow' ? 'bg-gradient-to-br from-yellow-400 to-orange-500' :
                              ach.color === 'blue' ? 'bg-gradient-to-br from-blue-400 to-indigo-500' :
                              ach.color === 'green' ? 'bg-gradient-to-br from-green-400 to-emerald-500' :
                              ach.color === 'red' ? 'bg-gradient-to-br from-red-400 to-pink-500' :
                              'bg-gradient-to-br from-purple-400 to-violet-500'
                            } shadow-lg transform hover:scale-110 transition-transform duration-300`}>
                              <Icon className="w-5 h-5 text-white" />
                            </div>
                            <div className="flex-1">
                              <p className="font-semibold text-gray-900 dark:text-white text-sm">{ach.title}</p>
                ...
                <Card className="relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900 dark:text-white">{quizzesCompleted}</p>
                    </div>
                  </div>
                </Card>
                ...
                <Card className="relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900 dark:text-white">{streakCount}</p>
                    </div>
                  </div>
                </Card>
              </motion.div>
                ...
                <Card>
                  ...
                  {results.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <h4 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No quizzes taken yet</h4>
                      <p className="text-gray-500 dark:text-gray-400 mb-4">Start your learning journey today!</p>
                      <Link to="/quizzes">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <Badge 
                              variant={
                                ...
                                (result.score / result.totalQuestions) >= 0.7 ? 'secondary' : 'destructive'
                              }
                            >
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Quiz Title</th>
                      ...
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Date Taken</th>
                      <th scope="col" className="relative px-6 py-3"><span className="sr-only">Actions</span></th>
                    </tr>
                  </thead>
                  <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                    {results.map((result) => (
                      <tr key={result._id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900 dark:text-white">{result.score} / {result.totalQuestions}</div>
                          <div className="text-sm text-gray-500 dark:text-gray-400">{((result.score / result.totalQuestions) * 100).toFixed(0)}%</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <Badge 
                            variant={
