...
export default function PDFQuizGenerator() {
  ...
  const generateAIQuestions = async () => {
    ...
    try {
      ...
      const apiUrl = `${import.meta.env.VITE_API_URL}/api/generate-pdf-questions`;
      ...
      setQuiz(prev => ({
        ...
        totalMarks: prev.totalMarks + formattedQuestions.reduce((sum, q) => sum + parseInt(q.marks), 0)
      }));

      alert(`Successfully generated ${formattedQuestions.length} AI questions!`);
  ...
  const parseAIResponse = (text) => {
    ...
    lines.forEach(line => {
      ...
      } else if (currentQuestion && (line.match(/^[A-D]\)/i) || line.match(/^[a-d]\./))) {
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      if (quiz.instructions) {
        ...
        const instructionLines = pdf.splitTextToSize(quiz.instructions, pageWidth - 40);
      ...
      quiz.questions.forEach((question, index) => {
        ...
        pdf.text(`[${question.marks} mark${question.marks > 1 ? 's' : ''}]`, pageWidth - 50, yPosition);
        
        yPosition += 8;

        // Question text
        pdf.setFont('helvetica', 'normal');
        const questionLines = pdf.splitTextToSize(question.question, pageWidth - 40);
        ...
        if (question.type === 'mcq') {
          question.options.forEach((option, optIndex) => {
            ...
            pdf.text(`${optionLetter}) ${option}`, 30, yPosition);
            yPosition += 8;
          });
      ...
      pdf.text('(For Teacher Use Only)', pageWidth / 2, yPosition + 10, { align: 'center' });
      ...
      quiz.questions.forEach((question) => {
        ...
        if (question.type === 'mcq') {
          ...
          answerText = `${correctLetter}) ${question.correctAnswer}`;
        } else if (question.type === 'truefalse') {
          answerText = question.correctAnswer;
        } else {
          answerText = question.explanation || 'Sample answer not provided';
        }
        ...
        if (question.explanation && question.type !== 'descriptive') {
          ...
          pdf.text(`Explanation: ${question.explanation}`, 50, yPosition);
          yPosition += 8;
          pdf.setFont('helvetica', 'normal');
        }
          ...
          <div className="inline-flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-2 flex items-center gap-2">
                ...
                Create printable quiz papers with AI assistance or manual control
              </p>
            </div>
          </div>
        </motion.div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={quiz.duration}
                    onChange={(e) => setQuiz(prev => ({ ...prev, duration: parseInt(e.target.value) || 60 }))}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                    min="1"
                  />
                </div>
              ...
              {showAIPanel ? (
                ...
                <div className="space-y-4 border-2 border-blue-200 dark:border-blue-800 rounded-lg p-4 bg-blue-50 dark:bg-blue-900">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-blue-900 dark:text-blue-100">AI Question Generator</h3>
                      <p className="text-sm text-blue-700 dark:text-blue-300">Let AI create questions for your topic</p>
                    </div>
                  </div>
                        ...
                        {['mcq', 'truefalse', 'descriptive'].map(type => (
                          <label key={type} className="flex items-center">
                            ...
                            <span className="text-sm capitalize">
                              {type === 'mcq' ? 'Multiple Choice' : type === 'truefalse' ? 'True/False' : 'Descriptive'}
                            </span>
                          </label>
                        ))}
                      </div>
                  ...
                  <Button 
                    ...
                    disabled={isGeneratingAI || !aiTopic.trim() || aiQuestionTypes.length === 0}
                    className="w-full"
                  >
                    {isGeneratingAI ? (
                      <>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-blue-50 dark:bg-blue-900 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{quiz.questions.length}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Questions</p>
                </div>
                <div className="text-center p-3 bg-green-50 dark:bg-green-900 rounded-lg">
                  <Star className="w-6 h-6 text-green-600 dark:text-green-400 mx-auto mb-1" />
                  <p className="text-2xl font-bold text-green-600 dark:text-green-400">{quiz.totalMarks}</p>
                ...
                <div className="text-center p-3 bg-orange-50 dark:bg-orange-900 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{quiz.duration}</p>
                ...
                <div className="text-center p-3 bg-purple-50 dark:bg-purple-900 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">PDF</p>
              ...
              <CardContent>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {quiz.questions.map((question, index) => (
                    <div key={question.id} className="flex items-start justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant={
                            question.type === 'mcq' ? 'default' :
                            question.type === 'truefalse' ? 'secondary' : 'outline'
                          }>
            ...
            >
              ...
              <div className="space-y-6">
                <div className="border-b pb-4">
                  <h1 className="text-xl font-bold">{quiz.title || 'Quiz Title'}</h1>
                ...
                {quiz.questions.map((question, index) => (
                  <div key={question.id} className="border-b pb-4">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-medium">Q{index + 1}. {question.question}</h3>
                    ...
                    {question.type === 'mcq' && (
                      <div className="space-y-1 ml-4">
                        {question.options.map((option, optIndex) => (
                          <p key={optIndex} className={`${option === question.correctAnswer ? 'text-green-600 font-medium' : ''}`}>
                    ...
                    {question.type === 'truefalse' && (
                      <div className="ml-4">
                        <p>True / False</p>
                        <p className="text-green-600 font-medium text-sm">Answer: {question.correctAnswer}</p>
                      </div>
                    )}
                    
                    {question.type === 'descriptive' && (
                      <div className="ml-4 space-y-2">
                        {Array.from({ length: Math.max(3, Math.ceil(question.marks / 2)) }).map((_, i) => (
