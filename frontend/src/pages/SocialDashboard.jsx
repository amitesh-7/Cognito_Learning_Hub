...
const SocialDashboard = () => {
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/users/search?query=${encodeURIComponent(searchQuery)}`,
        { headers: { 'x-auth-token': token } }
      );
      const data = await response.json();
      if (response.ok) {
  ...
  const respondToFriendRequest = async (friendshipId, action) => {
    try {
      const token = localStorage.getItem('quizwise-token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/friends/respond/${friendshipId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': token
        ...
        {searchResults.length > 0 && (
          <div className="space-y-2">
            ...
            {searchResults.map((searchUser) => (
              <div key={searchUser._id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div>
                  <p className="font-medium text-gray-800 dark:text-white">{searchUser.name}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{searchUser.email} • {searchUser.role}</p>
                </div>
                <div>
        ...
        {friends.length === 0 ? (
          ...
          <div className="grid gap-3">
            {friends.map((friendship) => (
              <div key={friendship.friendshipId} className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold">
                    {friendship.friend.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <p className="font-medium text-gray-800 dark:text-white">{friendship.friend.name}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {friendship.friend.role} • Friends since {new Date(friendship.since).toLocaleDateString()}
                    </p>
                  </div>
                </div>
        ...
        {challenges.length === 0 ? (
          ...
          <div className="space-y-4">
            {challenges.map((challenge) => (
              <div key={challenge._id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-700">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Gamepad2 className="w-5 h-5 text-purple-600 dark:text-purple-400" />
                    <span className="font-medium text-gray-800 dark:text-white">{challenge.quiz.title}</span>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    challenge.status === 'pending' ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200' :
                    challenge.status === 'accepted' ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200' :
                    challenge.status === 'completed' ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' :
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg p-3">
                    <div className="flex items-center gap-2 text-yellow-800 dark:text-yellow-200">
                      ...
                      <span className="font-medium">
                        Winner: {challenge.winner === user.id ? 'You!' : 
                                challenge.challenger._id === challenge.winner ? challenge.challenger.name : challenge.challenged.name}
                      </span>
                    </div>
                    {challenge.challengerResult && challenge.challengedResult && (
                      <div className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                        <p>{challenge.challenger.name}: {challenge.challengerResult.percentage}%</p>
                        <p>{challenge.challenged.name}: {challenge.challengedResult.percentage}%</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-full ${
                      notification.type === 'friend-request' ? 'bg-blue-100 dark:bg-blue-900' :
                      notification.type === 'challenge-won' ? 'bg-green-100 dark:bg-green-900' :
                      notification.type === 'quiz-challenge' ? 'bg-purple-100 dark:bg-purple-900' :
                      'bg-gray-100 dark:bg-gray-700'
                    }`}>
                      {notification.type === 'friend-request' && <UserPlus className="w-4 h-4 text-blue-600 dark:text-blue-400" />}
                      {notification.type === 'friend-accepted' && <Heart className="w-4 h-4 text-green-600 dark:text-green-400" />}
                      {notification.type === 'quiz-challenge' && <Gamepad2 className="w-4 h-4 text-purple-600 dark:text-purple-400" />}
                      {notification.type === 'challenge-won' && <Trophy className="w-4 h-4 text-yellow-600 dark:text-yellow-400" />}
                      {notification.type === 'broadcast' && <Radio className="w-4 h-4 text-red-600 dark:text-red-400" />}
                    </div>
                    <div>
                ...
                {notification.type === 'friend-request' && !notification.isRead && (
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        respondToFriendRequest(notification.metadata.friendshipId, 'accept');
                      }}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        respondToFriendRequest(notification.metadata.friendshipId, 'decline');
                      }}
  ...
  if (loading && !friends.length && !challenges.length && !notifications.length) {
    return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-800 dark:text-white mb-2">Social Hub</h1>
          <p className="text-gray-600 dark:text-gray-400">Connect with friends, compete in challenges, and stay updated!</p>
        </div>
