...
mongoose
  .connect(
    process.env.MONGODB_URI || "mongodb://localhost:27017/cognito_learning_hub",
    {
...
app.get("/api/admin/services/health", requireAdmin, async (req, res) => {
  try {
    ...
    const services = [
      ...
      ...service,
      url: service.url.endsWith("/health")
        ? service.url
        : `${service.url}/health`,
    }));
      ...
      services.map(async (service) => {
        try {
          ...
          return {
            ...
            uptime:
              response.data.uptime || response.data.uptimeSeconds || "N/A",
...
app.get("/api/admin/gemini/stats", requireAdmin, async (req, res) => {
  try {
    const quizServiceUrl =
      process.env.QUIZ_SERVICE_URL || "http://localhost:3003";
    const quizServiceHealth = await axios.get(`${quizServiceUrl}/health`, {
      timeout: 10000,
    });
...
app.post(
  "/api/admin/gemini/keys/:keyId/switch",
  requireAdmin,
  async (req, res) => {
    try {
      ...
      const response = await axios.post(
        `${quizServiceUrl}/api/admin/switch-api-key`,
        { keyId },
        { timeout: 10000 }
      );
...
app.post("/api/admin/gemini/keys", requireAdmin, async (req, res) => {
  try {
    ...
    const response = await axios.post(
      `${quizServiceUrl}/api/admin/add-api-key`,
      {
        apiKey,
...
app.delete("/api/admin/gemini/keys/:keyId", requireAdmin, async (req, res) => {
  try {
    ...
    await axios.delete(`${quizServiceUrl}/api/admin/remove-api-key/${keyId}`, {
      timeout: 10000,
    });
...
app.get("/api/admin/users/stats", requireAdmin, async (req, res) => {
  try {
    ...
    console.log(
      "Sample user for stats:",
      sampleUser
        ? { role: sampleUser.role, status: sampleUser.status }
        : "No users found"
    );

    const [totalUsers, activeUsers, students, teachers, moderators, admins] =
      await Promise.all([
        User.countDocuments(),
...
app.get("/api/admin/quizzes/stats", requireAdmin, async (req, res) => {
  try {
    ...
    console.log(
      "Sample quiz for stats:",
      sampleQuiz
        ? { status: sampleQuiz.status, difficulty: sampleQuiz.difficulty }
        : "No quizzes found"
    );
...
app.get("/api/admin/dashboard/stats", requireAdmin, async (req, res) => {
  try {
    ...
    const [totalUsers, activeUsers, totalQuizzes, pendingReports] =
      await Promise.all([
        User.countDocuments(),
        User.countDocuments({ status: "online" }),
        Quiz.countDocuments(),
...
const MICROSERVICES = {
  ...
  "Result Service": {
    url: process.env.RESULT_SERVICE_URL || "http://localhost:3003",
    port: 3003,
  },
  "Live Service": {
    url: process.env.LIVE_SERVICE_URL || "http://localhost:3004",
    port: 3004,
  },
  "Social Service": {
  ...
  "Gamification Service": {
    url: process.env.GAMIFICATION_SERVICE_URL || "http://localhost:3007",
    port: 3007,
  },
  "Moderation Service": {
...
app.post("/api/admin/services/logs", async (req, res) => {
  try {
    ...
    if (!serviceName || !message) {
      return res
        .status(400)
        .json({ error: "serviceName and message are required" });
    }
...
app.delete("/api/admin/services/logs/clear", requireAdmin, async (req, res) => {
  try {
    ...
    const ServiceLog = mongoose.connection.collection("servicelogs");
    const result = await ServiceLog.deleteMany({
...
app.get(
  "/api/admin/services/:serviceName/metrics",
  requireAdmin,
  async (req, res) => {
    try {
      const { serviceName } = req.params;
