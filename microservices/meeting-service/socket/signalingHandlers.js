...
module.exports = (io) => {
  io.on('connection', (socket) => {
    ...
    socket.on('join-meeting', async (data) => {
      try {
        const { roomId, userId, userName, userPicture, isVideoEnabled, isAudioEnabled } = data;
        ...
        if (!meeting) {
          ...
          const dbMeeting = await Meeting.findOne({ roomId, status: { $in: ['waiting', 'active'] } });
        ...
        logger.info(`User ${userId} joined meeting ${roomId} (${otherParticipants.length} existing participants)`);
    ...
    socket.on('webrtc-offer', async (data) => {
      try {
        ...
        logger.debug(`WebRTC offer from ${currentUserId} to ${targetUserId} in room ${roomId}`);
        
        // Get target participant
        const targetParticipant = await meetingManager.getParticipant(roomId, targetUserId);
        ...
        await meetingManager.addPeerConnection(roomId, currentUserId, targetUserId);
    ...
    socket.on('webrtc-answer', async (data) => {
      try {
        ...
        logger.debug(`WebRTC answer from ${currentUserId} to ${targetUserId} in room ${roomId}`);
        ...
        socket.emit('peer-connection-error', { error: 'Failed to send answer' });
      }
    });
    ...
    socket.on('ice-candidate', async (data) => {
      try {
        ...
        logger.debug(`ICE candidate from ${currentUserId} to ${targetUserId}`);
        
        // Get target participant
        const targetParticipant = await meetingManager.getParticipant(roomId, targetUserId);
        
        if (!targetParticipant || !targetParticipant.socketId) {
          return; // Silently ignore if peer not found (may have disconnected)
        }
    ...
    socket.on('toggle-audio', async (data) => {
      try {
        ...
        logger.debug(`User ${currentUserId} audio ${isEnabled ? 'enabled' : 'disabled'}`);
    ...
    socket.on('toggle-video', async (data) => {
      try {
        ...
        logger.debug(`User ${currentUserId} video ${isEnabled ? 'enabled' : 'disabled'}`);
    ...
    socket.on('toggle-screen-share', async (data) => {
      try {
        ...
        logger.info(`User ${currentUserId} screen sharing ${isSharing ? 'started' : 'stopped'}`);
    ...
    socket.on('change-video-quality', async (data) => {
      try {
        ...
        logger.debug(`User ${currentUserId} changed video quality to ${quality}`);
    ...
    socket.on('meeting-chat-message', async (data) => {
      try {
        ...
        const participant = await meetingManager.getParticipant(roomId, currentUserId);
    ...
    socket.on('leave-meeting', async (data) => {
      try {
        ...
        const participantCount = await meetingManager.getParticipantCount(roomToLeave);
      ...
      try {
        ...
        if (mapping && mapping.roomId) {
          ...
          if (participantCount === 0) {
            ...
            logger.info(`Meeting ${roomId} ended (no participants after disconnect)`);
          }
        }
