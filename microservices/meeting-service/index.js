...
app.get('/health', (req, res) => {
  res.json({
    ...
    status: 'healthy',
    redis: meetingManager.isConnected() ? 'connected' : 'disconnected',
...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, () => {
      ...
      logger.info(`STUN Servers: ${process.env.STUN_SERVERS || 'Not configured'}`);
      
      if (process.env.TURN_SERVER) {
        logger.info(`TURN Server: Configured`);
      } else {
        logger.warn(`TURN Server: Not configured (may cause connectivity issues behind strict NATs)`);
      }
    });
  } catch (error) {
