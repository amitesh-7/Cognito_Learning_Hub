...
const SERVICES = [
  ...
  { name: 'Gamification Service', url: 'https://gamification-service-ax6n.onrender.com' },
  { name: 'Quiz Service', url: 'https://quiz-service-6jzt.onrender.com' },
  { name: 'Moderation Service', url: 'https://moderation-service-3e2e.onrender.com' },
];
  ...
  return new Promise((resolve) => {
    ...
    const req = protocol.get(service.url, (res) => {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        console.log(`✅ [${timestamp}] ${service.name} - Status: ${res.statusCode} - Response time: ${duration}ms`);
        resolve({ success: true, service: service.name, statusCode: res.statusCode, duration });
      } else {
        console.log(`⚠️  [${timestamp}] ${service.name} - Status: ${res.statusCode} - Response time: ${duration}ms`);
        resolve({ success: false, service: service.name, statusCode: res.statusCode, duration });
      }
    ...
    req.on('error', (error) => {
      ...
      console.log(`❌ [${timestamp}] ${service.name} - Error: ${error.message} - Duration: ${duration}ms`);
      resolve({ success: false, service: service.name, error: error.message, duration });
    });
    ...
    req.on('timeout', () => {
      ...
      console.log(`⏱️  [${timestamp}] ${service.name} - Timeout after ${duration}ms`);
      resolve({ success: false, service: service.name, error: 'Timeout', duration });
    });
...
async function pingAllServices() {
  ...
  console.log('='.repeat(80));
  console.log(`📊 Summary: ${successful}/${results.length} services responded successfully`);
  console.log(`   Success: ${successful} | Failed: ${failed} | Avg Response Time: ${avgDuration}ms`);
